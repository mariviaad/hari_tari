import 'package:dotted_border/dotted_border.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:hari_tari/core/theme/constants/colors_const.dart';
import 'package:hari_tari/features/game/presentation/bloc/betting/betting_cubit.dart';
import 'package:hari_tari/features/game/presentation/bloc/betting/betting_state.dart';

class HeadsBetWidget extends StatelessWidget {
  const HeadsBetWidget({super.key});

  @override
  Widget build(BuildContext context) {
    final screenWidth = MediaQuery.of(context).size.width;
    final isSmallScreen = screenWidth < 1000;
    final double fontsizelg = screenWidth <= 600 ? 14 : 17.5;

    return BlocBuilder<BettingCubit, BettingState>(
      builder: (context, state) {
        final betAmount = state.betAmount;
        final totalHeads = state.totalHeadsBet;
        final yourHeadsBet = state.yourHeadsBet;
        final totalPool = state.totalHeadsBet + state.totalTailsBet;
        final winChance = totalPool > 0
            ? ((totalHeads / totalPool) * 100).toStringAsFixed(0)
            : "0";

        return Container(
          width: double.infinity,
          padding: EdgeInsets.all(isSmallScreen ? 15 : 20),
          decoration: BoxDecoration(
            color: const Color(0x331C398E),
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: const Color(0x4D2B7FFF), width: 1.5),
            boxShadow: [
              BoxShadow(
                color: const Color(0x402B7FFF),
                blurRadius: 10,
                spreadRadius: 1,
                blurStyle: BlurStyle.outer,
                offset: Offset.zero,
              ),
            ],
          ),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(
                "HEADS",
                style: TextStyle(
                  fontSize: fontsizelg,
                  fontWeight: FontWeight.bold,
                  color: const Color(0xFF8EC5FF),
                ),
              ),
              const SizedBox(height: 12),

              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: List.generate(3, (index) {
                  return Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 4),
                    child: DottedBorder(
                      options: CircularDottedBorderOptions(
                        dashPattern: [6, 4],
                        strokeWidth: 2,
                        color: const Color(0xFF8EC5FF),
                        padding: const EdgeInsets.all(0),
                      ),
                      child: CircleAvatar(
                        radius: 17,
                        backgroundColor: const Color(0x4D1C398E),
                        child: const Text(
                          "H",
                          style: TextStyle(
                            fontSize: 12.25,
                            fontWeight: FontWeight.bold,
                            color: Color(0xFF8EC5FF),
                          ),
                        ),
                      ),
                    ),
                  );
                }),
              ),
              const SizedBox(height: 14),
              Text(
                "₱$totalHeads",
                style: TextStyle(
                  fontSize: fontsizelg,
                  fontWeight: FontWeight.bold,
                  color: Colors.white,
                ),
              ),
              Text(
                "$winChance%",
                style: const TextStyle(
                  fontSize: 13.75,
                  color: Color(0xFF51A2FF),
                ),
              ),

              const SizedBox(height: 12),

              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Text(
                    "Your Bet: ",
                    style: TextStyle(fontSize: 12.25, color: Color(0xFF51A2FF)),
                  ),
                  Text(
                    "₱$yourHeadsBet",
                    style: TextStyle(
                      fontSize: 17.5,
                      fontWeight: FontWeight.bold,
                      color: AppColors.yellow,
                    ),
                  ),
                ],
              ),

              const SizedBox(height: 16),

              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  style: ButtonStyle(
                    backgroundColor: WidgetStateProperty.resolveWith<Color>((
                      Set<WidgetState> states,
                    ) {
                      if (states.contains(WidgetState.disabled)) {
                        return const Color(0x662B7FFF);
                      }
                      return const Color(0xFF2B7FFF);
                    }),
                    foregroundColor: WidgetStateProperty.all<Color>(
                      Colors.white,
                    ),
                    shape: WidgetStateProperty.all<RoundedRectangleBorder>(
                      RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(8),
                      ),
                    ),
                    padding: WidgetStateProperty.all<EdgeInsets>(
                      EdgeInsets.symmetric(vertical: isSmallScreen ? 12 : 14),
                    ),
                  ),
                  onPressed: (yourHeadsBet + betAmount > 200)
                      ? null
                      : () {
                          context.read<BettingCubit>().placeHeadsBet();
                        },
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(
                        (yourHeadsBet + betAmount > 200) ? "MAX BET" : "BET ",
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 12.25,
                        ),
                      ),
                      if (yourHeadsBet + betAmount <= 200)
                        Text(
                          "₱$betAmount",
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 12.25,
                          ),
                        ),
                    ],
                  ),
                ),
              ),

              if (yourHeadsBet + betAmount > 200)
                const Text(
                  "Would exceed maximum bet",
                  style: TextStyle(color: Color(0xFF99A1AF), fontSize: 10.5),
                ),
            ],
          ),
        );
      },
    );
  }
}
